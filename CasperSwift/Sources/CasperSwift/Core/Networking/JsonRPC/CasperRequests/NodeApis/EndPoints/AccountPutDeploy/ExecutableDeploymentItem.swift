...
public struct ExecutableDeploymentItem {

    public let type: ExecutableDeploymentItemType
    public let args: [Argument]
    
    public init(type: ExecutableDeploymentItemType, args: [Argument]) {
        self.type = type
        self.args = args
    }
    ...
    func asAnyEncodable() throws -> AnyEncodable {
        [
            type.stringValue: [
                "args": [
                    try args.map { try $0.asAnyEncodable() }
                ],
...
public enum ExecutableDeploymentItemType {
    case moduleBytes
    case transfer
